...
class NormalMotionPolicyMjlab(JointPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        self._kp = self._metadata_array(metadata, "joint_stiffness")
        self._kd = self._metadata_array(metadata, "joint_damping")
        self._action_scale = self._metadata_array(metadata, "action_scale")
        self._default_position = self._metadata_array(metadata, "default_joint_pos")
    ...
    def _validate_metadata(self, metadata: dict[str, str]) -> None:
        expected = (self.joint_contract.action.dof_num,)
        for name, value in (
            ("joint_stiffness", self._kp),
            ("joint_damping", self._kd),
            ("action_scale", self._action_scale),
            ("default_joint_pos", self._default_position),
        ):
            if value.shape != expected:
                raise ValueError(
                    f"model metadata {name} has shape {value.shape}, expected {expected}"
                )
    ...
    def step(
        ...
        *,
        ...
        np.subtract(joints.position, self._default_position, out=obs[6:35])
