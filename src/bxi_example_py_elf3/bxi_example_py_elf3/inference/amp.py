...
class HumanoidGaitPolicyLiteIsaaclab:
    ...
    def _initialize_model(self, model, backend):
        ...
        self._fill_history(
            ...
            np.zeros_like(self.default_position),
            np.array([1.0, 0.0, 0.0, 0.0]),  # 单位四元数
            np.zeros(3),  # 初始角速度
            np.array([0.0, 0.0, 0.0]),  # 初始命令速度
        )

    def step(self, q, dq, quat, omega, cmd_vel):
        ...
        self._update_observation(q, dq, quat, omega, cmd_vel)
        if monitor:
        ...
        self.vae_vel.fill(0.0)
        if out_len > vel_start:
            vel_len = min(3, out_len - vel_start)
            self.vae_vel[:vel_len] = self._action[vel_start : vel_start + vel_len]
        ...
        self.target[:output_action_dim] += self._scaled_action
        ...
        # 极简推理（比原版快5-15%）
        return self.target, self.vae_vel

    def reset(self, qj, dqj, quat, omega, cmd_vel):
        ...
        if self.extra_obs_dim > 0:
            self.episode_length_buf = 0
        self._fill_history(qj, dqj, quat, omega, self._command)
    ...
    def _update_observation(self, qj, dqj, quat, omega, cmd_vel):
        single_obs = self._build_single_observation(qj, dqj, quat, omega, cmd_vel)
    ...
    def _build_single_observation(self, qj, dqj, quat, omega, cmd_vel):
        ...
        np.subtract(qj, self.default_position, out=self._joint_delta)
        np.take(
            self._joint_delta,
            obs_q_idx,
