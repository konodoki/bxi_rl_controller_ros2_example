...
class _LegacyMotionPolicy(_MotionGeometry):
    def __init__(
        self,
        motion_npz_path: str,
        ...
        *,
        ...
        if isaac_order:
            ...
            self.default_position = self._default_policy_position[
                ISAAC_TO_MUJOCO
            ].copy()
            self.kp = ELF3_KPS.copy()
            self.kd = ELF3_KDS.copy()
            self._decode_scale = self._action_scale[ISAAC_TO_MUJOCO]
        else:
            self.kp = _csv_floats(meta["joint_stiffness"])
            self.kd = _csv_floats(meta["joint_damping"])
            self.default_position = self._default_policy_position
    ...
    def step(self, q, dq, quat, omega):
        ...
        self._build_input(q, dq, quat, omega)
    ...
    def _build_input(self, q, dq, quat, omega):
        ...
        if self._isaac_order:
            np.subtract(q, self.default_position, out=self._joint_delta)
...
class _HistoryMotionPolicy(_MotionGeometry):
    def __init__(
        self,
        motion_npz_path: str,
        model_onnx_path: str | ModelSpec,
        ...
        *,
        ...
        self.num_actions = len(self.joint_name)
        self._default_policy_position = _csv_floats(meta["default_joint_pos"])
        self.default_position = self._default_policy_position[ISAAC_TO_MUJOCO].copy()
        self._action_scale = _csv_floats(meta["action_scale"])
    ...
    def step(
        self, q, dq, quat, omega, base_lin_vel_b=None, robot_pos_w=None, advance=False
    ):
        monitor = self._runtime.options.monitor_enabled
        if monitor:
            started = time.perf_counter_ns()
        self._build_input(q, dq, quat, omega, base_lin_vel_b, robot_pos_w)
        self._time_input[0, 0] = int(self._frame)
        if monitor:
        ...
        np.take(self._target_isaac, ISAAC_TO_MUJOCO, out=self.target)
        if advance:
            self._frame = min(self._frame + 1.0, self._end_frame)
        if monitor:
    ...
    def _build_input(self, q, dq, quat, omega, base_lin_vel_b=None, robot_pos_w=None):
